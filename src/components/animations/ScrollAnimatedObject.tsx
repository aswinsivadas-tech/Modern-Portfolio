"use client";

import { useRef, useEffect } from "react";
import { Canvas } from "@react-three/fiber";
import { Float, Environment } from "@react-three/drei";
import * as THREE from "three";
import gsap from "gsap";
import { ScrollTrigger } from "gsap/ScrollTrigger";

gsap.registerPlugin(ScrollTrigger);

import { useMemo } from "react";
import { useFrame } from "@react-three/fiber";

function UnifiedNetwork() {
  const pointsRef = useRef<THREE.Points>(null);
  const linesRef = useRef<THREE.LineSegments>(null);

  const { positions, linePositions } = useMemo(() => {
    const particleCount = 200;
    const maxDistance = 0.7;
    const pos = new Float32Array(particleCount * 3);

    // Generate points on a sphere using Fibonacci lattice
    for (let i = 0; i < particleCount; i++) {
      const phi = Math.acos(-1 + (2 * i) / particleCount);
      const theta = Math.sqrt(particleCount * Math.PI) * phi;

      const r = 1.5;
      pos[i * 3] = r * Math.cos(theta) * Math.sin(phi);
      pos[i * 3 + 1] = r * Math.sin(theta) * Math.sin(phi);
      pos[i * 3 + 2] = r * Math.cos(phi);
    }

    const linePos = [];

    // Connect close points
    for (let i = 0; i < particleCount; i++) {
      for (let j = i + 1; j < particleCount; j++) {
        const dx = pos[i * 3] - pos[j * 3];
        const dy = pos[i * 3 + 1] - pos[j * 3 + 1];
        const dz = pos[i * 3 + 2] - pos[j * 3 + 2];
        const distSq = dx * dx + dy * dy + dz * dz;

        if (distSq < maxDistance * maxDistance) {
          linePos.push(
            pos[i * 3], pos[i * 3 + 1], pos[i * 3 + 2],
            pos[j * 3], pos[j * 3 + 1], pos[j * 3 + 2]
          );
        }
      }
    }

    return {
      positions: pos,
      linePositions: new Float32Array(linePos)
    };
  }, []);

  useFrame((_, delta) => {
    if (pointsRef.current && linesRef.current) {
      pointsRef.current.rotation.y += delta * 0.05;
      linesRef.current.rotation.y += delta * 0.05;
      pointsRef.current.rotation.x += delta * 0.02;
      linesRef.current.rotation.x += delta * 0.02;
    }
  });

  return (
    <group scale={1.2}>
      <points ref={pointsRef}>
        <bufferGeometry>
          <bufferAttribute
            attach="attributes-position"
            count={positions.length / 3}
            array={positions}
            itemSize={3}
            args={[positions, 3]}
          />
        </bufferGeometry>
        <pointsMaterial
          size={0.04}
          color="#0ea5e9"
          transparent
          opacity={0.9}
        />
      </points>
      <lineSegments ref={linesRef}>
        <bufferGeometry>
          <bufferAttribute
            attach="attributes-position"
            count={linePositions.length / 3}
            array={linePositions}
            itemSize={3}
            args={[linePositions, 3]}
          />
        </bufferGeometry>
        <lineBasicMaterial
          color="#38bdf8"
          transparent
          opacity={0.4}
          blending={THREE.AdditiveBlending}
        />
      </lineSegments>
    </group>
  );
}

function Scene() {
  const groupRef = useRef<THREE.Group>(null);
  const arrivalRef = useRef<THREE.Group>(null);

  useEffect(() => {
    const group = groupRef.current;
    const arrival = arrivalRef.current;
    if (!group || !arrival) return;

    // Use GSAP context to clean up easily
    const ctx = gsap.context(() => {
      // --- Arrival Animation ---
      // The object warps in from deep space and spins into place
      gsap.from(arrival.position, {
        z: -15,
        y: 8,
        duration: 3,
        ease: "power3.out",
      });

      gsap.from(arrival.rotation, {
        x: Math.PI * 3,
        y: Math.PI * 2,
        duration: 3.5,
        ease: "power2.out",
      });

      gsap.from(arrival.scale, {
        x: 0,
        y: 0,
        z: 0,
        duration: 2.5,
        ease: "back.out(1.5)",
      });

      // --- Scroll Animation ---
      // The timeline that spans the entire scroll height of the page
      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: document.documentElement,
          start: "top top",
          end: "bottom bottom",
          scrub: 1.5, // 1.5 seconds smoothing
        },
      });

      // Initial state: Top left, pushed slightly back
      group.position.set(-2.5, 1.5, -1);
      group.rotation.set(0, 0, 0);
      group.scale.set(1, 1, 1);

      // Waypoint 1: Move to bottom right
      tl.to(group.position, { x: 2.5, y: -1, z: 0, duration: 1 }, 0)
        .to(group.rotation, { x: Math.PI, y: Math.PI * 0.5, duration: 1 }, 0)

        // Waypoint 2: Move to center left (slightly forward)
        .to(group.position, { x: -2, y: 0.5, z: 1, duration: 1 }, 1)
        .to(group.rotation, { x: Math.PI * 2, y: Math.PI, duration: 1 }, 1)

        // Waypoint 3: Move to top right
        .to(group.position, { x: 2, y: 1.2, z: 0, duration: 1 }, 2)
        .to(group.rotation, { x: Math.PI * 3, y: Math.PI * 1.5, duration: 1 }, 2)

        // Waypoint 4: Move to bottom center
        .to(group.position, { x: 0, y: -1.5, z: 0.5, duration: 1 }, 3)
        .to(group.rotation, { x: Math.PI * 4, y: Math.PI * 2, duration: 1 }, 3)

        // Waypoint 5: End at center, scaled up slightly for the footer
        .to(group.position, { x: 0, y: 0, z: 2.5, duration: 1 }, 4)
        .to(group.rotation, { x: Math.PI * 5, y: Math.PI * 2.5, duration: 1 }, 4);
    });

    return () => ctx.revert();
  }, []);

  return (
    <group ref={arrivalRef}>
      <group ref={groupRef}>
        <Float speed={2} rotationIntensity={1.5} floatIntensity={1.5}>
          <UnifiedNetwork />
        </Float>
      </group>
    </group>
  );
}

export default function ScrollAnimatedObject() {
  return (
    <div className="fixed inset-0 z-0 pointer-events-none opacity-50">
      <Canvas camera={{ position: [0, 0, 5], fov: 45 }} gl={{ alpha: true }}>
        <ambientLight intensity={0.5} />
        <directionalLight position={[10, 10, 10]} intensity={1} />
        <Environment preset="city" />
        <Scene />
      </Canvas>
    </div>
  );
}



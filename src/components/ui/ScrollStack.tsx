...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      const cardTop = getElementOffset(card.parentElement as HTMLElement);
      ...
      let blur = 0;
      let opacity = 1;
      if (blurAmount || fadeAmount) {
        let topCardIndex = 0;
        for (let j = 0; j < cardsRef.current.length; j++) {
          const jCardTop = getElementOffset(cardsRef.current[j].parentElement as HTMLElement);
        ...
        if (i < topCardIndex) {
          ...
          opacity = Math.max(0, 1 - depthInStack * fadeAmount);
        }
      }
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      ...
      if (lenisRef.current && lenisRef.current !== (window as any).lenis) {
  ...
  if (useWindowScroll) {
    return (
      <div className={`scroll-stack-scroller ${className}`.trim()} ref={scrollerRef}>
        <div className="scroll-stack-inner w-full" style={{ paddingBottom: '100vh' }}>
          {children}
